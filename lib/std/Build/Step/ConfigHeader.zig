...
pub const default_header_text = "This file was generated by ConfigHeader using the Zig Build System.";
...
/// If left `null` the default header text is used.
/// Set to a non-null value to customize (eg. `"/* foo bar ... */\n"` or `""`).
/// Can't be `null` when `style` is `.blank`.
custom_header_text: ?[]const u8,
...
pub const Options = struct {
    ...
    /// If left `null` the default text is used
    /// Set to a non-null value to customize (eg. `"/* foo bar */\n"` or "")
    /// Can't be `null` for when `style` is `.blank`
    custom_header_text: ?[]const u8 = null,
};
...
pub fn create(owner: *std.Build, options: Options) *ConfigHeader {
    ...
    config_header.* = .{
        ...
        .custom_header_text = options.custom_header_text,
...
fn make(step: *Step, options: Step.MakeOptions) !void {
    ...
    man.hash.addOptionalBytes(config_header.custom_header_text);

    var output = std.ArrayList(u8).init(gpa);
    defer output.deinit();

    const c_generated_line = "/* " ++ default_header_text ++ " */\n";
    const asm_generated_line = "; " ++ default_header_text ++ "\n";

    if (config_header.custom_header_text) |text| {
        try output.appendSlice(text);
    } else {
        switch (config_header.style) {
            .autoconf, .cmake => try output.appendSlice(c_generated_line),
            .nasm => try output.appendSlice(asm_generated_line),
            .blank => @panic("`custom_header_text` can't be `null` when `style` is `.blank`"),
        }
    }
